...
public:
  ...
  virtual Ref<ATNConfig>
  precedenceTransition(Ref<ATNConfig> const &config,
                       const PrecedencePredicateTransition *pt,
...
protected:
  ...
  virtual std::unique_ptr<ATNConfigSet>
  computeStartState(ATNState *p, RuleContext *ctx, bool fullCtx);
  ...
  virtual std::vector<Ref<const SemanticContext>>
  getPredsForAmbigAlts(const antlrcpp::BitSet &ambigAlts, ATNConfigSet *configs,
                       size_t nalts);

  std::vector<dfa::DFAState::PredPrediction> getPredicatePredictions(
  ...
  /// </summary>
  antlrcpp::BitSet evalSemanticContext(
      const std::vector<dfa::DFAState::PredPrediction> &predPredictions,
   ...
   * @since 4.3
   */
  virtual bool evalSemanticContext(Ref<const SemanticContext> const &pred,
  ...
  virtual NoViableAltException
  noViableAlt(TokenStream *input, ParserRuleContext *outerContext,
