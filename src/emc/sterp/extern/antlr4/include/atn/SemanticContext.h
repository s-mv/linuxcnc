...
class ANTLR4CPP_PUBLIC SemanticContext
    : public std::enable_shared_from_this<SemanticContext> {
public:
   ...
   * </ul>
   */
  virtual Ref<const SemanticContext>
  evalPrecedence(Recognizer *parser, RuleContext *parserCallStack) const;
  ...
  static Ref<const SemanticContext> And(Ref<const SemanticContext> a,
                                        Ref<const SemanticContext> b);

  /// See also: ParserATNSimulator::getPredsForAmbigAlts.
  static Ref<const SemanticContext> Or(Ref<const SemanticContext> a,
                                       Ref<const SemanticContext> b);
...
protected:
  explicit SemanticContext(SemanticContextType contextType)
...
class ANTLR4CPP_PUBLIC SemanticContext::Predicate final
    : public SemanticContext {
public:
  static bool is(const SemanticContext &semanticContext) {
    return semanticContext.getContextType() == SemanticContextType::PREDICATE;
  }

  static bool is(const SemanticContext *semanticContext) {
    return semanticContext != nullptr && is(*semanticContext);
  }
...
class ANTLR4CPP_PUBLIC SemanticContext::PrecedencePredicate final
    : public SemanticContext {
public:
  static bool is(const SemanticContext &semanticContext) {
    return semanticContext.getContextType() == SemanticContextType::PRECEDENCE;
  }
  ...
  bool eval(Recognizer *parser, RuleContext *parserCallStack) const override;
  Ref<const SemanticContext>
  evalPrecedence(Recognizer *parser,
                 RuleContext *parserCallStack) const override;
  size_t hashCode() const override;
  bool equals(const SemanticContext &other) const override;
  std::string toString() const override;
};
...
public:
  static bool is(const SemanticContext &semanticContext) {
    ...
    return contextType == SemanticContextType::AND ||
           contextType == SemanticContextType::OR;
  }
  ...
  virtual const std::vector<Ref<const SemanticContext>> &
  getOperands() const = 0;
...
class ANTLR4CPP_PUBLIC SemanticContext::AND final
    : public SemanticContext::Operator {
public:
  static bool is(const SemanticContext &semanticContext) {
    return semanticContext.getContextType() == SemanticContextType::AND;
  }
 ...
 * contexts is true.
 */
class ANTLR4CPP_PUBLIC SemanticContext::OR final
    : public SemanticContext::Operator {
public:
  static bool is(const SemanticContext &semanticContext) {
    return semanticContext.getContextType() == SemanticContextType::OR;
  }
...
template <> struct hash<::antlr4::atn::SemanticContext> {
  size_t
  operator()(const ::antlr4::atn::SemanticContext &semanticContext) const {
