...
public:
  static bool is(const Transition &transition) {
    ...
    return transitionType == TransitionType::SET ||
           transitionType == TransitionType::NOT_SET;
  }
  ...
  SetTransition(ATNState *target, misc::IntervalSet set)
      : SetTransition(TransitionType::SET, target, std::move(set)) {}
...
protected:
  SetTransition(TransitionType transitionType, ATNState *target,
