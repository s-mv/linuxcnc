...
public:
   ...
   */
  template <class T> static size_t update(size_t hash, Ref<T> const &value) {
    return update(hash, value != nullptr ? value->hashCode() : 0);
  }

  template <class T> static size_t update(size_t hash, T *value) {
    return update(hash, value != nullptr ? value->hashCode() : 0);
  }
  ...
  static size_t update(size_t hash, const T *data, size_t size) {
    return update(hash, static_cast<const void *>(data),
  ...
  static size_t hashCode(const std::vector<Ref<T>> &data,
  ...
  static size_t hashCode(const void *data, size_t size,
                         size_t seed = DEFAULT_SEED) {
    size_t hash = initialize(seed);
  ...
  template <typename T>
  static size_t hashCode(const T *data, size_t size,
                         size_t seed = DEFAULT_SEED) {
    return hashCode(static_cast<const void *>(data),
                    size * sizeof(std::remove_reference_t<T>), seed);
  }
